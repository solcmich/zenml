...
def promote_challenger_to_champion(client: MlflowClient, station_id: str, parent_run_id: str) -> None:
    challenger_run = get_run_by_name(parent_run_id, station_id)
    if not challenger_run:
        logger.warning(f"No challenger run found for station {station_id}. Cannot promote.")
        return
    ...
    client.set_registered_model_alias(
        name=station_id, version=str(len(registered_models)), alias="champion",
    )
...
def validate_and_deploy_models(
    ...
    for station_id, model in trained_models.items():
        try:
            model_name = station_id  # Directly use station_id as model name
